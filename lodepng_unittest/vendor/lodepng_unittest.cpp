...
void createComplexPNG(std::vector<unsigned char>& png)
{
  ...
  std::cout << "enc status=" << error << std::endl;
...
void testPaletteFilterTypesZero()
{
  ...
  std::vector<unsigned char> png;
  createComplexPNG(png);
  std::cout << "testPaletteFilterTypesZero2" << std::endl;
...
void testChunkUtil()
{
  std::cout << "testChunkUtil-1" << std::endl;
  std::vector<unsigned char> png;
  createComplexPNG(png);
  std::cout << "testChunkUtil-crok" << std::endl;

  std::vector<std::string> names[3];
  std::vector<std::vector<unsigned char> > chunks[3];

  assertNoError(lodepng::getChunks(names, chunks, png));
  std::cout << "testChunkUtil-getok" << std::endl;
  ...
  std::cout << "testChunkUtil-insok" << std::endl;
...
extern "C" int lode_unittest_main1() {
  try {
    //PNG
    testPNGCodec();
    testPngSuiteTiny();
...
extern "C" int lode_unittest_main3() {
  try {
    //Colors
    testFewColors();
  } catch(...) {
...
extern "C" int lode_unittest_main4() {
  try {
    testColorKeyConvert();
    testColorConvert();
    testColorConvert2();
...
extern "C" int lode_unittest_main5() {
  try {
...
extern "C" int lode_unittest_main6() {
  try {
    testAutoColorModels1();
  } catch(...) {
    std::cout << "error!" << std::endl;
    return 1;
  }
...
extern "C" int lode_unittest_main7() {
  try {
    testNoAutoConvert();
  } catch(...) {
    std::cout << "error!" << std::endl;
